...
- **Global scope**: `/home/konrad/contact-monitor/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/konrad/contact-monitor/.claude/notes/`)
...
2. Glob `/home/konrad/contact-monitor/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/konrad/contact-monitor/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/konrad/contact-monitor/.claude/notes/` — works in any directory
