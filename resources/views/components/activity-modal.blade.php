...
<div id="activity-modal-overlay"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 relative">
        <button onclick="closeActivityModal()"
                class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 text-lg leading-none">✕</button>
...
function openActivityModal(btn) {
    ...
    fetch(src, {headers: {'X-Requested-With': 'XMLHttpRequest'}})
        .then(r => r.text())
        .then(html => { body.className = 'overflow-y-auto max-h-[80vh]'; body.innerHTML = html; })
