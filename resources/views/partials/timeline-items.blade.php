    ...
    <div class="flex items-center justify-end py-1.5 pr-2 min-w-0 overflow-hidden">
        @if($d->isCustomer)
            <div class="flex items-center gap-1.5 max-w-full group rounded-lg px-2 py-1 transition-colors min-w-0
                ...
                {{-- Icon --}}
                @if($d->chType)
                    <span @if($d->badgeTitle) title="{{ $d->badgeTitle }}" @endif class="shrink-0">
                ...
                @if($d->isEmail)
                    <div class="min-w-0 flex-1 flex flex-col gap-px text-right">
                        ...
                        <div class="flex items-center justify-end gap-1 min-w-0">
                            ...
                            @if($d->sourcePerson)
                                <a href="{{ route('people.show', $d->sourcePerson) }}" class="text-[10px] text-brand-600 hover:underline truncate max-w-[120px] md:max-w-[260px]">{{ $d->sourceLabel }}</a>
                            @else
                                <span class="text-[10px] text-gray-500 truncate max-w-[120px] md:max-w-[260px]">{{ $d->sourceLabel }}</span>
                            @endif
                        </div>
                        @endif
                        @if($d->counterpartLabel)
                        <div class="flex items-center justify-end gap-1 min-w-0">
                            <span class="text-[10px] text-gray-400 shrink-0">{{ $d->isOutbound ? 'from:' : 'to:' }}</span>
                            @if(($showPersonLink ?? false) && $activity->person)
                                <a href="{{ route('people.show', $activity->person) }}" class="text-[10px] text-brand-600 hover:underline truncate max-w-[120px] md:max-w-[260px]">{{ $d->counterpartLabel }}</a>
                            @elseif(($showCompanyLink ?? false) && $activity->company)
                                <a href="{{ route('companies.show', $activity->company) }}" class="text-[10px] text-brand-600 hover:underline truncate max-w-[120px] md:max-w-[260px]">{{ $d->counterpartLabel }}</a>
                            @else
                                <span class="text-[10px] text-gray-500 truncate max-w-[120px] md:max-w-[260px]">{{ $d->counterpartLabel }}</span>
                            @endif
                        </div>
                        @endif
                            ...
                            @if($d->titleText)
                                @if($d->modalUrl)
                                    <button type="button" data-modal-src="{{ $d->modalUrl }}" onclick="openActivityModal(this)"
                                            class="text-xs link truncate max-w-[120px] md:max-w-[260px] text-right cursor-pointer" title="{{ $d->hoverText }}">{{ $d->titleText }}</button>
                                @elseif($d->url)
                                    <a href="{{ $d->url }}" class="text-xs link truncate max-w-[120px] md:max-w-[260px] text-right" title="{{ $d->hoverText }}">{{ $d->titleText }}</a>
                                @else
                                    <span class="text-xs text-gray-600 truncate max-w-[120px] md:max-w-[260px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</span>
                                @endif
                            @endif
                ...
                @elseif($d->isSlack || $d->isDiscord)
                    <div class="min-w-0 flex-1 flex flex-col gap-px text-right">
                        <div class="flex items-center gap-1.5 min-w-0 justify-end">
                            @if($d->sourceLabel)
                                <span class="text-xs text-gray-400 shrink-0 font-mono truncate max-w-[100px] md:max-w-[200px]" title="{{ $d->sourceLabel }}">{{ $d->sourceLabel }}</span>
                            @endif
                            @if(($showPersonLink ?? false) && $activity->person)
                ...
                @elseif($d->isTicket)
                    <div class="min-w-0 flex-1 flex flex-col gap-px text-right">
                        <div class="flex items-center gap-1.5 min-w-0 justify-end">
                            @if($d->ticketNotFound)
                                <span class="text-xs text-red-400 truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText ?: '#' . $d->ticketNotFound }}</span>
                            @elseif($d->titleText && $d->url)
                                <a href="{{ $d->url }}" class="text-xs link truncate max-w-[140px] md:max-w-[300px] text-right" title="{{ $d->hoverText }}">{{ $d->titleText }}</a>
                            @elseif($d->titleText)
                                <span class="text-xs text-gray-600 truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</span>
                            @endif
                            <span class="text-xs text-gray-400 shrink-0 tabular-nums whitespace-nowrap"
                                  title="{{ $activity->occurred_at->format('d M Y H:i') }}">{{ $activity->occurred_at->diffForHumans(null, true, true) }}</span>
                        </div>
                ...
                @else
                    ...
                    @if($d->titleText)
                        @if($d->modalUrl && !$d->rowClickable)
                            <button type="button" data-modal-src="{{ $d->modalUrl }}" onclick="openActivityModal(this)"
                                    class="text-xs link truncate max-w-[140px] md:max-w-[300px] text-right cursor-pointer" title="{{ $d->hoverText }}">{{ $d->titleText }}</button>
                        @elseif($d->ticketNotFound)
                            <span class="text-xs text-red-500 truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</span>
                        ...
                        @endif
                    @elseif($d->ticketNotFound)
                        <span class="text-xs text-red-400 font-mono truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">#{{ $d->ticketNotFound }}</span>
                    @endif
                    <div class="flex-1 min-w-0"></div>
                    @if(($showPersonLink ?? false) && $activity->person)
            ...
            <div class="flex items-center gap-1.5 max-w-full group rounded-lg px-2 py-1 transition-colors min-w-0
                    ...
                    <div class="min-w-0 flex-1 flex flex-col gap-px">
                        ...
                        <div class="flex items-center gap-1.5 min-w-0">
                            @if($d->titleText)
                                @if($d->modalUrl)
                                    <button type="button" data-modal-src="{{ $d->modalUrl }}" onclick="openActivityModal(this)"
                                            class="text-xs link truncate max-w-[120px] md:max-w-[260px] text-left cursor-pointer" title="{{ $d->hoverText }}">{{ $d->titleText }}</button>
                                @elseif($d->url)
                                    <a href="{{ $d->url }}" class="text-xs link truncate max-w-[120px] md:max-w-[260px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</a>
                                @else
                                    <span class="text-xs text-gray-600 truncate max-w-[120px] md:max-w-[260px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</span>
                                @endif
                            @endif
                    ...
                    <div class="min-w-0 flex-1 flex flex-col gap-px">
                        <div class="flex items-center gap-1.5 min-w-0">
                            @if($d->ticketNotFound)
                                <span class="text-xs text-red-400 truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText ?: '#' . $d->ticketNotFound }}</span>
                            @elseif($d->titleText && $d->url)
                                <a href="{{ $d->url }}" class="text-xs link truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</a>
                            @elseif($d->titleText)
                                <span class="text-xs text-gray-600 truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</span>
                            @endif
                            <div class="flex-1 min-w-0"></div>
                ...
                @else
                    ...
                    @if($d->titleText)
                        @if($d->modalUrl && !$d->rowClickable)
                            <button type="button" data-modal-src="{{ $d->modalUrl }}" onclick="openActivityModal(this)"
                                    class="text-xs link truncate max-w-[140px] md:max-w-[300px] text-left cursor-pointer" title="{{ $d->hoverText }}">{{ $d->titleText }}</button>
                        @elseif($d->ticketNotFound)
                            <span class="text-xs text-red-500 truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</span>
                        @elseif($d->url && !$d->modalUrl)
                            <a href="{{ $d->url }}" class="text-xs link truncate max-w-[140px] md:max-w-[300px]" title="{{ $d->hoverText }}">{{ $d->titleText }}</a>
