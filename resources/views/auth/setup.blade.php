...
<head>
    ...
    <title>Set up — Contact Monitor</title>
...
<div class="w-full max-w-sm">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 px-8 py-8">
        <h1 class="text-base font-semibold text-gray-900 mb-1">Create your admin account</h1>
        <p class="text-sm text-gray-500 mb-6">No accounts exist yet. Set up your administrator login.</p>

        @if($errors->any())
            <div role="alert" class="mb-5 px-4 py-3 bg-red-50 border border-red-200 text-red-800 rounded-lg text-sm flex items-start gap-2">
        ...
        <form method="POST" action="{{ route('setup.post') }}" class="space-y-4">
            @csrf
            ...
            <div>
                <label class="label" for="password">Password <span class="text-gray-400 font-normal">(min. 8 characters)</span></label>
