...
@php
    $currentView = request('view', 'unlinked');
    $currentQ    = request('q', '');
    $currentTab  = request('tab', 'people');
@endphp
    ...
    <div class="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
        <a href="{{ request()->fullUrlWithQuery(['view' => 'unlinked', 'page' => null]) }}"
           class="px-4 py-1.5 font-medium transition
                  {{ $currentView === 'unlinked' ? 'bg-amber-50 text-amber-700 border-r border-gray-200' : 'bg-white text-gray-500 hover:bg-gray-50 border-r border-gray-200' }}">
            Unlinked <span class="ml-1 font-bold">{{ $unlinkedCount }}</span>
        </a>
        <a href="{{ request()->fullUrlWithQuery(['view' => 'linked', 'page' => null]) }}"
           class="px-4 py-1.5 font-medium transition
                  {{ $currentView === 'linked' ? 'bg-green-50 text-green-700' : 'bg-white text-gray-500 hover:bg-gray-50' }}">
