<form action="{{ $action }}" method="POST" class="flex gap-2 items-center">
    @csrf
    <div class="relative flex-1" data-autocomplete="person" data-search-url="{{ route('people.search') }}">
        <input type="text" placeholder="Search person…" autocomplete="off"
               class="ac-input text-xs border border-gray-300 rounded px-2 py-1 w-full focus:outline-none focus:border-brand-400">
        <input type="hidden" name="person_id" class="ac-value">
