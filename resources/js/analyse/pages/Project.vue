<template>
  <div class="flex h-full">
    ...
    <main class="flex-1 overflow-y-auto p-8 lg:ml-0 ml-0">
      ...
      <div class="max-w-2xl">
        ...
        <div class="flex items-center gap-3 mb-6">
          <div v-if="!renaming" class="flex items-center gap-2">
            ...
            <button
              class="text-gray-500 hover:text-gray-300 text-xs px-2 py-1 rounded"
              @click="startRename"
            >Rename</button>
          </div>
          <div v-else class="flex items-center gap-2 flex-1">
            <input
              ref="renameInput"
              v-model="renameValue"
              class="bg-gray-800 border border-gray-700 text-gray-200 rounded px-3 py-1 text-sm flex-1"
              @blur="commitRename"
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-3 text-gray-200">
                  {{ chat.title }}
                  <span v-if="chat.pinned" class="ml-1 text-xs text-amber-400" title="Pinned">&#9733;</span>
    ...
    <Teleport to="body">
      <div v-if="confirmDeleteModal" class="fixed inset-0 flex items-center justify-center z-50 bg-black/40" @click.self="confirmDeleteModal = false">
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-5 w-72 shadow-xl">
          <h3 class="text-sm font-semibold text-gray-200 mb-2">Delete project?</h3>
          <p class="text-xs text-gray-400 mb-4">Conversations will be unassigned, not deleted.</p>
...
async function unpin(chatId) {
  await fetch(`/analyse/projects/${props.project.id}/pin-chat/${chatId}`, {
    method: 'DELETE',
...
async function commitRename() {
  ...
  await fetch(`/analyse/projects/${props.project.id}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken() },
